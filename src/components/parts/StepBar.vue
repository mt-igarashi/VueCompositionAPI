<template>
<div v-if="data.show" :class="mainCss">
  <ol class="c-stepper">
...
import { Step } from "@/js/commonType";
 ...
 */
   ...
   * 引数：index インデックス
   */
  const getItemCss = (index:number):string[] => {
   ...
   * 引数：index インデックス
   */
  const getTitleCss = (index:number):string[] => {
    if (props.step == index) {
   ...
   * 引数：index インデックス
   */
  const getDescCss = (index:number):string[] => {
    if (props.step == index) {
