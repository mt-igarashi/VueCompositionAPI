export default class Validator {
   ...
   * 関数概要: コンストラクタ
   */
  constructor(vue: any, settings: any) {
    this.vue = vue;
    this.settings = settings;
    this.messageStore = new Map<string, string[]>();
    this.callbackStore = new Map<string, Function>();
  }
   ...
   * 引数：clear 全削除フラグ
   */
  initializeMessegeStore(clear:boolean = false):void {
    if (clear) {
   ...
   * 引数：field フィールド名
   */
  ConvertErrorMessage(field:string = ""):any {
    if (field) {
   ...
   */
  addCallbackStore(field:string, callback:Function):void {
   ...
   *        colindex 列インデックス
   */
  validateMinLength(map:any):void {
    const pairInfo = this.getPairInfo(map);
    if (!pairInfo.value && pairInfo.value !== 0) {
      return;
    }
   ...
   *        colindex 列インデックス
   */
  validateMaxLength(map:any):void {
    const pairInfo = this.getPairInfo(map);
    if (!pairInfo.value && pairInfo.value !== 0) {
      return;
    }
   ...
   * 引数：message エラーメッセージ
   */
  getCommonMessage(message:string):string {
    switch(message) {
      case "COM-0001": {
        return "{0}は必須入力項目です。";
      }
   ...
   */
  getIndexedFieldName(field:string, index?:number):string {
    if (index == null) {
      return field;
    }
   ...
   */
  getVueData(field:string, property:string = ""):any {
    if (!property) {
   ...
   */
  formatMessage(message:any, ...args:string[]):string {
    let result = message;
